function [ Acc ] = main_KJMLC_CRC( param )
...
for ii1 = 1:nTest
    ...
    er = ones(mm,1);
    tt.P = inv(K_YY + er*er' + lambda2*eye(mm));
    %%%%---end------------------------------------------------------
    ...
    for ii2 = 1:nTrain
        ...
        e_eta = ones(2*n_XXi1_sam+2,1);
        ntr.P = inv(K_XuXu + e_eta*e_eta' + lambda1*eye( 2*n_XXi1_sam+2 ));
        ntr.K_XuY = K_XuY;
        ntr.K_XiY = K_XiY;
        ntr.K_UiY = K_UiY;
        dist = myL2C_Correct_fast(K_YY, K_XuXu, B1, B21, B24, B41, B44, lambda1,lambda2,ntr,tt);
        Dis(ii2) = dist;
    end
    %%%------------------------end----------------------------------------%
